...
export default function createAdminAdsRouter(supabase: SupabaseClient) {
  const router = Router();
  ...
  router.get('/auth/:provider', (req, res) => {
    ...
    const redirectUri = `${process.env.VITE_API_URL}/api/admin/ads/callback/${provider}`;
  ...
  router.get('/callback/:provider', async (req, res) => {
    ...
    try {
      // Aquí iría la lógica de intercambio de código por token
