...
def send_admin_notification(session):
    ...
    if not smtp_user or not smtp_password or not admin_emails_str:
        print(" No se puede enviar notificación a socios: faltan credenciales.")
        return
...
def create_event(business_id, session):
    db = SessionLocal()
    try:
        # Crear instancia con campos básicos seguros
        new_booking = Booking(
            business_id=business_id,
        ...
        # Asignar campos extra dinámicamente SOLO si el modelo Booking los soporta
...
def handle_chat(business_id: str, session_id: str, message: str):
    ...
    # Usar Ollama Cloud
    response = cloud_chat(
        model=OLLAMA_MODEL,  # modelo definido en configuración
        messages=messages
    )
    ...
    if session.get("intent") == "check_availability":
        ...
        # Usamos una sesión nueva y limpia para esta consulta
